    ...
    if ($this->rg_total > $this->rg_limit) {
      ...
      if ($this->pg_total <= $this->interval) {
        for ($i = 1; $i <= $this->pg_total; $i++) {
          if ($this->pg_actual != $i)
            $this->html .= "<span><a href=\"{$this->link}/$i\">$i</a></span>";
          else
            $this->html .= "<span id='actual'>$i</span>";
        }
      }
      ...
      elseif ($this->pg_actual > ($this->pg_total - $this->half)) {
        ...
        if ($this->half * 2 == $this->interval)
          $final = $this->pg_actual + $this->half;
        else
          $final = $this->pg_actual + $this->half - 1;
        for ($i; $i <= $final; $i++) {
          if ($this->pg_actual != $i)
            $this->html .= "<span><a href=\"{$this->link}/$i\">$i</a></span>";
          else
